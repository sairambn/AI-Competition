...
export function ProblemsPage() {
  ...
  const copyProblem = async () => {
    if (!result) return;
    const text = `Team: ${result.teamName}\nProblem: ${result.problem.title}\n\n${result.problem.description}\n\nDifficulty: ${formatDifficulty(result.difficulty)}\nCategory: ${result.problem.category}\nTeam size: ${result.teamSize}`;
    await navigator.clipboard.writeText(text);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const registerUrl = result
    ? `https://github.com/sairambn/AI-Competition/issues/new?title=${encodeURIComponent(`[Assignment] ${result.teamName} — ${result.problem.title}`)}&body=${encodeURIComponent(`## Team Assignment\n\n- **Team name:** ${result.teamName}\n- **Team size:** ${result.teamSize}\n- **Problem:** ${result.problem.title}\n- **Difficulty:** ${formatDifficulty(result.difficulty)}\n- **Category:** ${result.problem.category}\n- **Problem ID:** ${result.problem.id}\n\n### Description\n${result.problem.description}\n\n---\n*Registered via AI Problem Solve-a-Thon site*`)}`
    : "#";
            ...
            <div className="hero-card-glow rounded-xl border border-gold/40 bg-parchment p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <a href={registerUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  <CheckCircle2 className="h-4 w-4" /> Register Assignment (required)
                </a>
                ...
                <button type="button" onClick={() => { setResult(null); setCopied(false); }}
                  className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-secondary">
                  <RefreshCcw className="h-4 w-4" /> Generate Another
                </button>
                <Link to={`/submit?team=${encodeURIComponent(result.teamName || "")}&problem=${result.problem.id}`}
                  className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-secondary">
                  Go to Submit <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
              <p className="mt-3 text-xs text-muted-foreground">
                Click <strong>Register Assignment</strong> so the organizer can see your team + problem on GitHub Issues.
              </p>
            </div>
          </div>
        )}
